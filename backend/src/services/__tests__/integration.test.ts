...
describe('Integration Tests - Multi-Website Parallel Retrieval', () => {
  ...
  it('should continue processing other websites when one fails', async () => {
    ...
    axios.get = jest.fn().mockImplementation((url) => {
      // Make the second website always fail
      if (url.includes('failing-site.com')) {
    ...
    // Should have results for all websites (even failed ones)
    expect(result.results).toHaveLength(3);
    
    // At least one should fail
    expect(result.failedCount).toBeGreaterThanOrEqual(1);
    
    // At least one should succeed (error tolerance - continues processing)
    ...
    // Verify successful results have keyword matches
...
describe('Integration Tests - Content Retrieval and Analysis', () => {
  ...
  it('should generate summary document with sources', async () => {
    ...
    // Re-create analysis service with proper mock that returns the LLM content
    const mockSummaryRepo = {
      create: jest.fn().mockImplementation(async (data) => ({
        id: 'summary-123',
        executionId: data.executionId,
        content: data.content, // Return the actual content passed in
